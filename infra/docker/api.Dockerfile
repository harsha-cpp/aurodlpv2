...
#
# Auro Healthcare DLP — FastAPI API image.
#
# BUILD CONTEXT MUST BE THE REPO ROOT:
#   docker build -f infra/docker/api.Dockerfile -t aurodlp/api:dev .
...
#
# Two stages: `builder` owns the compilers and the uv cache, `runtime` gets only
# the resolved virtualenv plus the shared libraries the detection engine dlopen()s.
...
# Python rather than curl: the interpreter is already here, and adding curl to a
# healthcare image is attack surface for a request we can make in one line.
...
CMD ["uvicorn", "aurodlpv2_backend.main:app", "--host", "0.0.0.0", "--port", "8000", "--proxy-headers", "--forwarded-allow-ips", "*"]
