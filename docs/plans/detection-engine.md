# MedShield Detection Engine — Build Plan
...
### 2.1 Module layout (Python package `medshield_detection`)
```
medshield_detection/
├── __init__.py
...
- PaddleOCR: optional extra (`pip install medshield-detection[paddle]`). The router degrades gracefully if PaddleOCR is not importable — logs a warning and skips fallback. This keeps the lean Tesseract-only image small for tenants who don't need Indic/handwriting.
...
- `medshield_detection` package, `pyproject.toml`, ruff + mypy + pytest baseline.
