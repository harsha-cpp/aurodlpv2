...
|---|---|---|
| Entity precision / recall / F1 | 0.608 / 0.291 / 0.393 | 1.000 / 0.965 / 0.982 |
| Document precision / recall / F1 | 0.796 / 0.513 / 0.624 | 1.000 / 0.987 / 0.993 |
...
Nothing downstream is verifiable without this. A labelled corpus of 116
documents / 172 entity spans across `clinical`, `administrative`, `identifiers`,
...
- **Server-side Gmail enforcement.** By decision, the extension is the only
  ...
  or the Gmail API.
- **Part two (LLM / textbox DLP).** Not in this repository. It should be built
  on the Phase 1 rule pack rather than as a third independent detector.
...
- No end-to-end test drives a real Gmail session; the extension's send-path
  interception is unit-tested against synthetic DOM only.
