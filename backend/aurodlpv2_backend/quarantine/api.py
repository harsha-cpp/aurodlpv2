...
from aurodlpv2_backend.db.models import QuarantineItem
from aurodlpv2_backend.deps import (
    CurrentMember,
    DbSession,
    ExtensionPrincipal,
    QuarantineReviewer,
...
async def _get_item(
    ...
    *,
    ...
    statement = select(QuarantineItem).where(
        QuarantineItem.id == item_id,
        QuarantineItem.org_id == org_id,
    )
    if for_update:
...
async def list_quarantine_items(
    ...
    session: DbSession,
    status_filter: Annotated[QuarantineStatus | None, Query(alias="status")] = "pending",
...
async def approve_quarantine_item(
    ...
    item = await _get_item(session, item_id, member.org_id, for_update=True)
...
async def extension_quarantine_status(
    ...
    item = await _get_item(session, item_id, extension.org_id)
