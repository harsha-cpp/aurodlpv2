"""TOTP enrolment and the second step of login."""
...
class EnrollResponse(BaseModel):
    #: Feed straight into a QR code. Contains the shared secret, so it is
...
class ConfirmResponse(BaseModel):
    #: Shown once. Only hashes are stored, so a lost set is regenerated by
    #: re-enrolling, never recovered.
...
async def enroll_mfa(member: CurrentMember, session: DbSession) -> EnrollResponse:
    ...
    if record is not None and record.confirmed_at is not None:
        # Re-enrolling silently would replace a working authenticator with an
        # unscanned one; disable first so the swap is deliberate.
    ...
    if record is None:
        ...
        # Unconfirmed row: the member restarted enrolment, so the old secret is
        # worthless and must not linger.
...
async def verify_mfa(
    ...
    return await issue_session(
        session, response, db_member, org, request=request, mfa_enabled=True
    )
...
def _decrypt_or_409(record: MemberMfa) -> str:
    try:
        ...
        # The encryption key changed under us. Re-enrolment is the only path
        # back, and saying so beats an opaque 500.
        raise HTTPException(
...
def _accepts_code(record: MemberMfa, code: str) -> bool:
    """A live TOTP code, or one of the single-use backup codes."""
    secret = _decrypt_or_409(record)
