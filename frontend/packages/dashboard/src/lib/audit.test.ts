...
import { actionLabel, applyFilters, auditCsv, categoryLabel, distinctActors, verifyChain } from './audit';
...
// Rows arrive newest-first, so rows[i]'s predecessor is rows[i + 1].
const intact = [
...
describe('verifyChain', () => {
  ...
  it('refuses to judge a filtered list, because hidden rows are not tampering', () => {
    expect(verifyChain(intact, false)).toEqual(['unverifiable', 'unverifiable', 'root']);
  });
...
describe('applyFilters', () => {
  ...
  it('filters by category', () => {
    expect(applyFilters(rows, { category: 'policy', actor: '' }).map((r) => r.id)).toEqual(['1', '3']);
  });

  it('matches actors case-insensitively on a substring', () => {
    expect(applyFilters(rows, { category: '', actor: 'BOB' }).map((r) => r.id)).toEqual(['2']);
  });

  it('applies both filters together', () => {
    expect(applyFilters(rows, { category: 'policy', actor: 'device' }).map((r) => r.id)).toEqual(['3']);
  });
});
...
describe('distinctActors', () => {
  it('lists each actor once, sorted, so the filter dropdown is stable', () => {
    ...
    expect(distinctActors(rows)).toEqual(['member:ann@x.test', 'member:bob@x.test']);
  });
});
