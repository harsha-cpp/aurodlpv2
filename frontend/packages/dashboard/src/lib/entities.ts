/**
 * The canonical PHI/PII vocabulary (detection/evaluation/taxonomy.py).
 * A ward clerk reviewing a quarantined message should read "Patient visit ID",
 * not `PATIENT_VISIT_ID`.
 */
...
/**
 * Falls back to a de-shouted version of the raw type: the detection engine can
 * ship a new recogniser before this map learns about it, and a readable
 * "Some new id" beats a blank cell.
 */
export function entityLabel(type: string | null | undefined): string {
  if (!type) return 'Unknown type';
  if (isKnown(type)) return ENTITY_META[type].label;
  const words = type.replace(/^IN_/, '').replace(/_/g, ' ').toLowerCase().trim();
