...
const three = [rule('block-blocked'), rule('quarantine-external'), rule('allow-rest')];

describe('renumber', () => {
  it('derives order from list position, because position is what the engine walks', () => {
    expect(renumber(three).map((r) => r.order)).toEqual([ORDER_STEP, ORDER_STEP * 2, ORDER_STEP * 3]);
  });
...
describe('moveRule', () => {
  it('moves a rule up and renumbers so first match still means first row', () => {
    ...
    expect(moved.map((r) => r.id)).toEqual(['block-blocked', 'allow-rest', 'quarantine-external']);
    expect(moved.map((r) => r.order)).toEqual([10, 20, 30]);
  });

  it('moves a rule down', () => {
    const moved = moveRule(renumber(three), 0, 1);
    expect(moved.map((r) => r.id)).toEqual(['quarantine-external', 'block-blocked', 'allow-rest']);
  });
  ...
  it('round-trips: up then down restores the original order', () => {
    const start = renumber(three);
    expect(moveRule(moveRule(start, 1, -1), 0, 1).map((r) => r.id)).toEqual(start.map((r) => r.id));
  });
});

describe('inEvaluationOrder', () => {
  it('sorts by order, so a server set with sparse numbers still reads correctly', () => {
    const rules = [rule('c', { order: 90 }), rule('a', { order: 10 }), rule('b', { order: 50 })];
  ...
  it('breaks ties by original position instead of reordering arbitrarily', () => {
    ...
    expect(inEvaluationOrder(rules).map((r) => r.id)).toEqual(['first', 'second']);
  });
});
...
describe('validateRules', () => {
  ...
  it('catches an unsatisfiable risk window', () => {
    const problems = validateRules([rule('impossible', { conditions: { min_risk_score: 80, max_risk_score: 20 } })]);
  ...
  it('warns when everything is switched off, which would allow every send', () => {
    ...
    expect(problems.some((p) => p.includes('every send would be allowed'))).toBe(true);
  });
});
...
describe('isDirty', () => {
  ...
  it('is true when a rule is disabled', () => {
    expect(isDirty(saved, updateRule(renumber(three), 'allow-rest', { enabled: false }))).toBe(true);
  });

  it('ignores empty-vs-absent condition arrays, which the engine treats alike', () => {
    const withEmpty = renumber(three).map((r) => ({ ...r, conditions: { entity_types_any: [] } }));
...
describe('describeConditions', () => {
  ...
  it('uses readable entity labels, not raw type names', () => {
    const text = describeConditions({ entity_types_any: ['PATIENT_VISIT_ID'] }).join(' ');
  ...
  it('distinguishes "any recipient" from "every recipient"', () => {
    expect(describeConditions({ recipient_class_any: ['blocked'] })[0]).toContain('any recipient');
    expect(describeConditions({ recipient_class_all: ['internal'] })[0]).toContain('every recipient');
  });

  it('renders both sides of a risk window', () => {
    expect(describeConditions({ min_risk_score: 30, max_risk_score: 70 })).toEqual([
      'risk ≥ 30',
      'risk ≤ 70',
    ]);
  });
  ...
  it('describes both attachment states', () => {
    expect(describeConditions({ has_attachments: true })).toEqual(['has attachments']);
    expect(describeConditions({ has_attachments: false })).toEqual(['has no attachments']);
  });
});

describe('versionForSave', () => {
  it('renames the built-in set on first customisation', () => {
    expect(versionForSave({ version: 'builtin-2026.08', rules: three, is_custom: false })).toBe('custom');
  });

  it('keeps an organization’s own version name on later saves', () => {
    expect(versionForSave({ version: 'ward-policy-v3', rules: three, is_custom: true })).toBe('ward-policy-v3');
  });
