...
import { analyticsCsv, bucketOfAction, buildTrend, interventionRate, SERIES } from './analytics';
...
describe('buildTrend', () => {
  ...
  it('fills quiet days with zeros so the chart does not bridge the gap', () => {
    const trend = buildTrend([{ day: '2026-08-28', action: 'block', count: 2 }], 7, now);
  ...
  it('sums the per-action rows for a day into the right buckets', () => {
    ...
    expect(trend[1]).toMatchObject({ allowed: 10, warned: 3, stopped: 3, total: 16 });
  });
  ...
  it('tolerates a full timestamp where a date was expected', () => {
    const trend = buildTrend([{ day: '2026-08-28T00:00:00Z', action: 'allow', count: 4 }], 1, now);
...
describe('interventionRate', () => {
  it('is the share of scans that were stopped', () => {
    expect(
      interventionRate({ total_scans: 200, total_blocks: 10, total_quarantines: 5, total_escalations: 5 }),
    ).toBe(10);
  });

  it('is null rather than NaN when nothing has been scanned', () => {
    expect(
      interventionRate({ total_scans: 0, total_blocks: 0, total_quarantines: 0, total_escalations: 0 }),
    ).toBeNull();
  });
});
...
describe('SERIES palette', () => {
  it('keeps crimson reserved for the stopped series', () => {
    expect(SERIES.stopped).toBe('#dc2626');
