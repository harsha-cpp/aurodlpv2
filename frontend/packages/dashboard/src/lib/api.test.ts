...
describe('request error path', () => {
  ...
  it('throws an ApiError carrying the status and the server detail', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(403, { detail: 'insufficient role' })));
  ...
  it('keeps a structured detail intact so callers can branch on it', async () => {
    const detail = { code: 'org_selection_required', organizations: [{ id: '1' }] };
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(409, { detail })));

    const err = await request('/api/v1/auth/login', { method: 'POST', skipAuth: true }).catch((e: unknown) => e);
  ...
  it('falls back to the status text when the body is not JSON', async () => {
    ...
    const err = (await request('/api/v1/events/analytics').catch((e: unknown) => e)) as ApiError;
  ...
  it('retries once with a refreshed token after a 401', async () => {
    const fetchMock = vi
      .fn()
      // The original call, rejected because the in-memory token expired.
      .mockResolvedValueOnce(jsonResponse(401, { detail: 'expired' }))
      // The refresh itself.
      .mockResolvedValueOnce(jsonResponse(200, { access_token: 'fresh-token' }))
      // The replay, which succeeds.
      .mockResolvedValueOnce(jsonResponse(200, { ok: true }));
    vi.stubGlobal('fetch', fetchMock);
    setAccessToken('stale-token');

    await expect(request<{ ok: boolean }>('/api/v1/members')).resolves.toEqual({ ok: true });
    expect(fetchMock).toHaveBeenCalledTimes(3);

    const replayCall = fetchMock.mock.calls[2] as unknown[] | undefined;
    const replayHeaders = (replayCall?.[1] as RequestInit | undefined)?.headers as
  ...
  it('does not attempt a refresh for a request that opted out of auth', async () => {
    ...
    await expect(request('/api/v1/auth/login', { method: 'POST', skipAuth: true })).rejects.toBeInstanceOf(ApiError);
  ...
  it('drops undefined query values instead of sending the string "undefined"', async () => {
    ...
    await request('/api/v1/quarantine', { query: { status: undefined, limit: 100 } });
...
describe('errorMessage', () => {
  it('uses the server string when there is one', () => {
    expect(errorMessage(new ApiError(403, 'insufficient role'))).toBe('insufficient role');
  });
  ...
  it('flattens a FastAPI validation array into something a person can act on', () => {
    ...
    expect(errorMessage(err)).toBe('password: String should have at least 12 characters');
  });

  it('never renders an object as [object Object]', () => {
    const err = new ApiError(409, { code: 'org_selection_required' });
    expect(errorMessage(err)).toBe('org_selection_required');
    expect(errorMessage(new ApiError(500, {}))).not.toContain('[object Object]');
  });
