...
export default function RuleEditor({
  ...
  function setCondition<K extends keyof RuleConditions>(
    key: K,
    ...
    >
      ...
      >
        <div className="field grow" style={{ minWidth: 200 }}>
          <label className="label" htmlFor={`id-${rule.id}`}>
            Rule id
          </label>
          <input
            id={`id-${rule.id}`}
          ...
          />
          <span className="hint">
            Appears in scan verdicts and quarantine records.
          </span>
        </div>
        <div className="field" style={{ minWidth: 160 }}>
          <label className="label" htmlFor={`action-${rule.id}`}>
            Action
          </label>
          <select
            id={`action-${rule.id}`}
            ...
            value={rule.action}
            onChange={(e) =>
              onChange({ action: e.target.value as PolicyRule["action"] })
            }
          >
        ...
        <div className="field" style={{ minWidth: 160 }}>
          <label className="label" htmlFor={`sev-${rule.id}`}>
            Report severity as at least
          </label>
          <select
            id={`sev-${rule.id}`}
            className="select"
            ...
            onChange={(e) =>
              onChange({
                min_reported_severity: e.target.value
      ...
      <div className="field">
        <label className="label" htmlFor={`desc-${rule.id}`}>
      ...
      <div className="field">
        <label className="label" htmlFor={`msg-${rule.id}`}>
      ...
      >
        ...
        <div className="row gap-3" style={{ flexWrap: "wrap" }}>
          ...
          <div className="field" style={{ minWidth: 150 }}>
            <label className="label" htmlFor={`minsev-${rule.id}`}>
              Min severity
            </label>
            <select
              id={`minsev-${rule.id}`}
          ...
          <div className="field" style={{ minWidth: 150 }}>
            <label className="label" htmlFor={`att-${rule.id}`}>
              Attachments
            </label>
            <select
              id={`att-${rule.id}`}
              className="select"
              value={
                c.has_attachments === undefined || c.has_attachments === null
                  ? ""
                  : String(c.has_attachments)
              }
              onChange={(e) =>
                setCondition(
                  "has_attachments",
