  ...
  if (!token) {
    return (
      <Shell title="Reset link incomplete">
        <p className="muted">
          This page needs the token from the email we sent. Open the link directly from that email,
  ...
  if (done) {
    return (
      <Shell title="Password updated">
        <p className="muted">
          Your password has been changed and every other session was signed out. Sign in with the new
          password.
        </p>
        <button
          type="button"
      ...
      <form onSubmit={onSubmit} className="col gap-4" style={{ marginTop: 24 }}>
        <div className="field">
          ...
          <input
            ...
            autoFocus
          />
          <span className="hint">At least {MIN_PASSWORD} characters. Common passwords are rejected.</span>
        </div>
        <div className="field">
