...
async function extractPdf(file: File): Promise<string> {
  ...
  } as Parameters<typeof lib.getDocument>[0]);
  try {
...
export async function resolveAttachmentRefs(
  ...
  return fetched.filter((file): file is File => Boolean(file));
}

export async function scanAttachmentRefs(
  refs: AttachmentUrlRef[],
): Promise<EntityHit[]> {
  if (refs.length === 0) return [];
  const files = await resolveAttachmentRefs(refs);
