...
const types = (text: string): string[] => detectPhi(text).map((hit) => hit.type);
...
describe('rule pack parity with the server', () => {
  ...
  it('names entities the way the server does', () => {
    // The old fallback said ABHA_ID and ICD10_CODE; the server said ABHA and
    // ICD10, and the policy had to list both spellings to compensate.
    expect(types('ABHA 12-3456-7890-1234 linked')).toContain('ABHA_NUMBER');
    expect(types('Primary diagnosis coded as N39.0 for the patient')).toContain('ICD10');
  });
});

describe('false-positive traps', () => {
  it('does not read a room number or a vitamin as a diagnosis', () => {
    expect(types('Clinical meeting in room A12 about the patient')).not.toContain('ICD10');
    expect(types('Patient advised vitamin B12 supplements')).not.toContain('ICD10');
  });

  it('does not read a vendor invoice as a health ID', () => {
    expect(types('Please process vendor invoice 12345678901234.')).not.toContain('ABHA_NUMBER');
  });

  it('does not read a purchase order as a record number', () => {
    expect(types('Order HSP-2026-0012 for stationery has been delivered.')).not.toContain('MRN');
  });

  it('does not read lowercase prose as a person', () => {
    expect(types('the patient in bed 7 became oliguric overnight')).not.toContain('PERSON');
  });
});
...
describe('risk scoring', () => {
  ...
  it('ranks several distinct identifiers above one repeated identifier', () => {
    const repeated = scorePhi(detectPhi(Array(5).fill('Aadhaar 7534 7930 7460').join(' ')));
    const distinct = scorePhi(
      detectPhi(
        ['7534 7930 7460', '7919 8193 9197', '8534 4210 7545', '2199 2000 1839', '9552 4877 0811']
