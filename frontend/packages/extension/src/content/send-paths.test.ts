...
describe("classifying a single control", () => {
  it("recognises the Send button in the shapes Gmail labels it", () => {
    expect(
      classifySendElement(el('<div role="button" aria-label="Send"></div>')),
    ).toBe("send");
    expect(
      classifySendElement(
        el('<div role="button" aria-label="Send ‪(⌘Enter)‬"></div>'),
      ),
    ).toBe("send");
    expect(
      classifySendElement(
        el('<div role="button" data-tooltip="Send (Ctrl-Enter)"></div>'),
      ),
    ).toBe("send");
    expect(
      classifySendElement(
        el('<div role="button" aria-label="Send &amp; Archive"></div>'),
      ),
    ).toBe("send");
  });
  ...
  it("treats the Schedule send menu item as a send, not an exemption", () => {
    expect(
      classifySendElement(
        el('<div role="menuitem" aria-label="Schedule send"></div>'),
      ),
    ).toBe("schedule-open");
    expect(
      classifySendElement(el('<div role="menuitem">Schedule send</div>')),
    ).toBe("schedule-open");
  });
  ...
  it("treats the confirm button in the schedule dialog as a send", () => {
    expect(
      classifySendElement(
        el('<button aria-label="Schedule send">Schedule send</button>'),
      ),
    ).toBe("schedule-confirm");
  });

  it("leaves discard alone - throwing a draft away is not a send", () => {
    expect(
      classifySendElement(
        el('<div role="button" aria-label="Discard draft"></div>'),
      ),
    ).toBeNull();
  });
  ...
  it("ignores the arrow that only opens the send menu", () => {
    expect(
      classifySendElement(
        el('<div role="button" aria-label="More send options"></div>'),
      ),
    ).toBeNull();
  });
  ...
  it("ignores unrelated controls that merely mention sending", () => {
    expect(
      classifySendElement(
        el('<div role="menuitem" aria-label="Send feedback to Google"></div>'),
      ),
    ).toBeNull();
    expect(
      classifySendElement(
        el('<div role="button" aria-label="Save draft"></div>'),
      ),
    ).toBeNull();
  });
...
describe("the schedule dialog", () => {
  ...
  it('treats a preset time as a commitment even though it never says "send"', () => {
    const root = dialog(
      '<button class="preset">Tomorrow morning 8:00 AM</button>',
    );
  ...
  it("does not claim buttons in unrelated dialogs", () => {
    const root = el(
      '<div role="dialog" aria-label="Insert link"><button class="ok">OK</button></div>',
    );
  ...
  it("reads the dialog name from its heading when there is no aria-label", () => {
    ...
    expect(findSendTrigger(root.querySelector(".preset"))?.kind).toBe(
      "schedule-confirm",
    );
  });
});
...
describe("walking up from the clicked node", () => {
  it("finds the button when the click lands on an inner span", () => {
    const button = el(
      '<div role="button" aria-label="Send"><span class="inner">Send</span></div>',
    );
  ...
  it("returns nothing for an ordinary click in the message body", () => {
    const body = el(
      '<div role="textbox" aria-label="Message Body"><p class="p">hello</p></div>',
    );
...
describe("keyboard send paths", () => {
  it("matches Gmail's documented send shortcut on both platforms", () => {
    expect(
      isSendShortcut({ key: "Enter", ctrlKey: true, metaKey: false }),
    ).toBe(true);
    expect(
      isSendShortcut({ key: "Enter", ctrlKey: false, metaKey: true }),
    ).toBe(true);
    expect(
      isSendShortcut({ key: "Enter", ctrlKey: false, metaKey: false }),
    ).toBe(false);
    expect(isSendShortcut({ key: "a", ctrlKey: true, metaKey: false })).toBe(
      false,
    );
  });
  ...
  it("treats Enter and Space on a focused control as activation", () => {
    expect(
      isActivationKey({ key: "Enter", ctrlKey: false, metaKey: false }),
    ).toBe(true);
    expect(isActivationKey({ key: " ", ctrlKey: false, metaKey: false })).toBe(
      true,
    );
    expect(
      isActivationKey({ key: "Escape", ctrlKey: false, metaKey: false }),
    ).toBe(false);
  });
