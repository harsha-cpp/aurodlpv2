...
export function mountWarningModal(
  ...
  const pollQuarantine =
    verdict.quarantine_id && orgCode
      ? async () => {
          // The modal polls this every 3s; without a deadline a hung request
          // stacks up connections behind a status that never arrives.
          const controller = new AbortController();
          const timer = setTimeout(() => controller.abort(), QUARANTINE_POLL_TIMEOUT_MS);
          try {
